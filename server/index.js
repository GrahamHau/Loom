...
app.get("/api/auth/feishu/callback", asyncHandler(async (req, res) => {
  ...
  const sessionUser = buildSessionUser(sessionUserResponse(localUser), profile);
...
function startRssScheduler() {
  ...
  }, Number(process.env.RSS_SCHEDULER_CHECK_INTERVAL_MS || process.env.RSS_COLLECT_INTERVAL_MS || 15 * 60 * 1000)).unref();
}
