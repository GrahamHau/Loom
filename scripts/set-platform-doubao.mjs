...
//   DOUBAO_VISION_MODEL   默认 doubao-seed-2-0-mini-260428（读图最快，实测 ~2.5s，卖点更干净）
//                         可一键换：mini(最快) / lite / pro(别漏)——纯 DB 配置、不改代码。
...
const config = {
  ...
  vision_model: (process.env.DOUBAO_VISION_MODEL || "doubao-seed-2-0-mini-260428").trim(),
  api_key: apiKey,
